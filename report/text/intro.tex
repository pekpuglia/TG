...
The proposed scenarios in this work are a circular-to-circular rendez-vous problem and a non-coplanar rendez-vous in multiple revolutions. The goal is to validate the results under the two body model with the results for the known Hohmann transfer~\cite{curtis2015orbital} and previous results in the primer vector field, as in~\citeonline{interactive_primer_vector}. Then, the same scenarios are to be solved under perturbed orbital models, including oblateness effects and drag, extending the domain of the primer vector method to new models.
