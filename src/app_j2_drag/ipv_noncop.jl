...
f
##
save_with_views("results/j2drag/ipv_noncop/$(transfer_type(solved_transfer_2r))", orb1, orb2, solved_transfer_2r, imp_sc)
##
...
f
##
save("./results/j2drag/ipv_noncop/$(transfer_type(solved_transfer_2r))_primer_vector.png", f, px_per_unit = 300/96)
##
tableICI = transfer_table(solved_transfer_2r, tspan_ppdot_stm, L, T, tolepsilon, NAME)
dump_table("./report/text/resultados.tex", "% J2D NCOP ICI", tableICI)
##
serialize("./src/app_j2_drag/ncop_ICI", solved_transfer_2r)
###########################################################################################################################################
###########################################################################################################################################
###########################################################################################################################################
...
##
seq0 = [scale(s, L, T) 
    for s = initial_orb_sequence(orb1, tf_real, N_2, 2, true, true, [0.6203643833757418, 0.32133012446092846, 0.0583054921633297])
]
...
f
##
save_with_views("results/j2drag/ipv_noncop/$(transfer_type(solved_transfer_2f))", orb1, orb2, solved_transfer_2f, imp_sc)
##
...
f
##
save("./results/j2drag/ipv_noncop/$(transfer_type(solved_transfer_2f))_primer_vector.png", f, px_per_unit = 300/96)
##
tableCICIC = transfer_table(solved_transfer_2f, tspan_ppdot_stm, L, T, tolepsilon, NAME)
dump_table("./report/text/resultados.tex", "% J2D NCOP 2 CICIC", tableCICIC)
##
serialize("./src/app_j2_drag/ncop_CICIC", solved_transfer_2f)
###########################################################################################################################################
###########################################################################################################################################
###########################################################################################################################################
...
solver_3 = casadi.nlpsol("s", "ipopt", planner_3.prob, Dict("ipopt" => Dict(
    ...
    "max_resto_iter" => 200)))
##
best_random_transfer, best_part3 = random_starts(solver_3, planner_3, transfer_3, orb1, tf_real, L, T, 30)

total_dV(best_random_transfer)
##
seq0 = [scale(s, L, T) 
    for s = initial_orb_sequence(orb1, tf_real, N_3, 3, true, true, [0.22709548417788206, 0.48792303541925675, 0.186791358563553, 0.0981901218393082])
]
...
f
##
save_with_views("results/j2drag/ipv_noncop/$(transfer_type(solved_transfer_3))", orb1, orb2, solved_transfer_3, imp_sc)
##
...
f
##
save("./results/j2drag/ipv_noncop/$(transfer_type(solved_transfer_3))_primer_vector.png", f, px_per_unit = 300/96)
##
tableCICICIC = transfer_table(solved_transfer_3, tspan_ppdot_stm, L, T, tolepsilon, NAME)
dump_table("./report/text/resultados.tex", "% J2D NCOP 3 CICICIC", tableCICICIC)
##
sum_tab = summary_table(NAME, solved_transfer_2r, solved_transfer_2f, solved_transfer_3)
dump_table("./report/text/resultados.tex", "% J2D NCOP SUM", sum_tab)
##
serialize("./src/app_j2_drag/ncop_CICICIC", solved_transfer_3)
##
###########################################################################################################################################
###########################################################################################################################################
###########################################################################################################################################
...
# f
# ##
# save_with_views!(ax3d, f, "results/j2drag/ipv_noncop/$(transfer_type(solved_transfer_4))")
# ##
...
# f
# ##
# save("./results/j2drag/ipv_noncop/$(transfer_type(solved_transfer_4))_primer_vector.png", f, px_per_unit = 300/96)
# ##
