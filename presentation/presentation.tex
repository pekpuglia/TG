...
\title[LEO Primer Vector]{Primer Vector Analysis of Optimal Impulsive Orbital Maneuvers Under Conservative and Non-Conservative Perturbations}
...
\begin{frame}
    \frametitle{Dynamical systems tools}

    \begin{itemize}
        \item Full ODE for double integrator:
        \begin{equation}
            ...
            \end{bmatrix}
        \end{equation}\pause
        \item \(\acc(\pos, \vel)\) depends on model: \(\acc_{TB}\), \(\acc_{J2}\), \(\acc_{D}\)\pause
        \item State transition matrix (STM) for variational perturbations~\cite{montenbruck2000satellite}
            \begin{equation}
                \boldsymbol{\Phi}_\delta(t) = \frac{\partial \x(t)}{\partial \x_0}
            \end{equation}\pause
        \item STM differential equation
        \begin{equation}
            \begin{cases}
                \dot{\boldsymbol{\Phi}}_\delta(t) = \frac{\partial f(\x(t))}{\partial x(t)} \boldsymbol{\Phi}_\delta(t) = \mathbf{A}_\delta(t) \boldsymbol{\Phi}_\delta(t) \\
                \boldsymbol{\Phi}_\delta(0) = \mathbf{I}_n
            \end{cases}
        \end{equation}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \pause
    \begin{itemize}
        \item Decreasing cost sequence~\cite{ross2015primer}:
        \begin{enumerate}
            \item Feasible solution
            \begin{itemize}
                \item Satisfies constraints
                \item Found in nonlinear solver
            \end{itemize}\pause
            \item Local optimum
            \begin{itemize}
                \item Satisfies constraints, better than neighbors
                \item Found in nonlinear optimizer
            \end{itemize}\pause
            \item PMP extremum
            \begin{itemize}
                \item Local optimum + Pontryagin's Minimum Principle
            \end{itemize}\pause
            \item Global optimum
            \begin{itemize}
                \item Exists, almost impossible to characterize
            \end{itemize}\pause
            \item Lower bound
            \begin{itemize}
                \item Infeasible solution, still useful
                \item Problem intuition: Jupiter maneuvering \cite{impulsive_europa}
                \item Mathematical structure: sum-of-squares~\cite{sos_book}
            \end{itemize}
        \end{enumerate}
    \end{itemize}
...
\begin{frame}
    ...
    \pause
    \begin{itemize}
        \item Impulsive thrust\pause
        \begin{itemize}
            \item Engineering: independent of propulsive plant\pause
            \item Physics: high thrust \(\approx\) impulse\pause
            \item Mathematics: minimum is unattainable with bounded acceleration~\cite{impulsive_control}\pause
        \end{itemize}
        \item Velocity discontinuities, \textit{coasting arcs}
        \begin{equation}
            \boldsymbol{\Gamma}(t_i) = \Delta \mathbf{v} \delta(t_i) \implies \mathbf{v}(t_i^+) = \mathbf{v}(t_i^-) + \Delta \mathbf{v}
        \end{equation}\pause
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \end{equation}\pause
        \item \((1 + \boldsymbol{\lambda}_v^T \hat{\mathbf{u}})\) as small as possible when \(\Gamma > 0\)\pause
        \item \(\therefore \hat{\uc} \parallel - \boldsymbol{\lambda}_v\)\pause
        \item Define primer vector \(\mathbf{p} = - \boldsymbol{\lambda}_v\): optimal direction of impulse\pause
        ...
        \end{itemize}\pause
        \item Suboptimal trajectory:\pause
        \begin{itemize}
            \item (\(\lVert \mathbf{p}(0) \rVert = 1\)) and \(\partial_t \lVert \mathbf{p} \rVert \mid_{t=0} > 0\), adding an initial coast will lower the cost;\pause
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \end{equation}
        \begin{itemize}
            \item \textbf{ODE method}: numerical integration\pause
            \item Linear ODE: Primer vector transition matrix (PVTM) \(\Phi_p(t)\)\pause
            \item Same as ODE for variational perturbations: use state transition matrix (STM) \(\Phi_\delta(t)\)\pause
            \item \textbf{STM method}: \(\Phi_p(t) = \Phi_\delta(t) = \frac{\partial \x(t)}{\partial \x_0}\)\pause
            \item Keplerian model:
            \begin{itemize}
                \item \textbf{Glandorf method}: analytical form of \(\Phi_p(t) = \Phi_\delta(t)\)~\cite{glandorf_transition_matrix}
            \end{itemize}\pause
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \end{equation}
        \begin{itemize}
            \item Not found in the literature!\pause
            \item \textbf{Not} the same as variational perturbation ODE\pause
            \item Only \textbf{ODE method} available\pause
        \end{itemize}
    \end{itemize}
...
\begin{frame}
    \frametitle{Bibliography review}

    \begin{itemize}
        \item Orbit propagation
        \begin{itemize}
            \item \texttt{SatelliteToolbox}~\cite{satellitetoolbox}: fast implementation of semi-analytical propagators
            \begin{itemize}
                \item Explicit propagation with control flow: unsuited for optimization\pause
            \end{itemize}
            \item Modified Equinoctial Elements~\cite{mod_equinoctial_els}: singularity-free orbital elements
            \begin{itemize}
                \item Used extensively in finite thrust literature
            \end{itemize}\pause
        \end{itemize}
        \item Primer Vector
        \begin{itemize}
            \item Introduction in 1960s~\cite{lawden_optimal_1963}\pause
            \item STM method: Jupiter maneuvers~\cite{impulsive_europa}, asteroid encounter~\cite{bruno_asteroid_primer}\pause
            \item Interactive algorithm for number of impulses~\cite{interactive_primer_vector}\pause
            \item \textbf{Missing}: primer vector in non-conservative LEO environment
        \end{itemize}
    \end{itemize}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item \texttt{SatelliteToolbox}~\cite{satellitetoolbox}: auxiliary orbit propagation
        \item CasADi~\cite{casadi}: optimization interface
        \item Ipopt~\cite{ipopt}: robust nonlinear optimizer. Local, deterministic, gradient-based
    \end{itemize}
...
\begin{frame}
    ...
    \begin{itemize}\pause
        ...
        \end{itemize}\pause
        \item Impulse: boundary condition between coasting arcs\pause
        \item Coasting arc
        \begin{itemize}
        ...
        \end{itemize}\pause
        \item Search for \textbf{number of impulses}~\cite{interactive_primer_vector}\pause
        \begin{enumerate}
...
\begin{table}[htbp]
    ...
    \begin{itemize}
        ...
        \item Keplerian Model
        \begin{itemize}
            \item Circle to circle: Hohmann transfer
            \item Noncoplanar rendez-vous: Solved in~\cite{interactive_primer_vector}
        \end{itemize}
        \item Both solved for Keplerian, J2, J2+Drag
    \end{itemize}
\end{table}
% C2C SETUP

\end{frame}
%TODO put previous total
\begin{frame}
    ...
    \begin{columns}
        ...
        \begin{column}{0.49\linewidth}
            \begin{figure}[htbp]
                ...
                \caption{\texttt{CICIC}, view from z+}
                % \label{<label>}
            \end{figure}
        \end{column}
    \end{columns}
    ...
    \begin{columns}
        ...
        \begin{column}{0.49\linewidth}
            \begin{figure}[htbp]
                ...
                \caption{\texttt{CICICIC}, view from z+}
                % \label{<label>}
            \end{figure}
        \end{column}
    \end{columns}
    ...
    \begin{columns}
        ...
        \begin{column}{0.49\linewidth}
            \begin{figure}[htbp]
                ...
                \caption{\texttt{CICICICIC}, view from z+}
                % \label{<label>}
            \end{figure}
        \end{column}
    \end{columns}
...
\begin{frame}
    ...
    \begin{columns}
        ...
        \begin{column}{0.3\linewidth}
            \begin{itemize}
                \item Keplerian optimum: \texttt{ICI}
                \item Hohmann transfer
                \item \(\Delta v = 887.56\) m/s
            \end{itemize}
        \end{column}
    \end{columns}
    ...
    \begin{columns}[t]
        \begin{column}{0.7\linewidth}
            \only<3>{
                \begin{figure}[htbp]
                    \centering
                    \includegraphics[width=\linewidth]{../results/j2drag/hohmann/ICI_3d.png}  
                    \caption{\texttt{ICI}}
                \end{figure}
            }
            ...
            \only<4>{
                \begin{figure}[htbp]
                    \centering
                    \includegraphics[width=\linewidth]{../results/j2drag/hohmann/CICIC_3d.png}  
                    \caption{\texttt{CICIC}}
                \end{figure}
            }
            ...
            \only<5>{
                \begin{figure}[htbp]
                    \centering
                    \includegraphics[width=\linewidth]{../results/j2drag/hohmann/CICICIC_3d.png}  
                    \caption{\texttt{CICICIC}}
                \end{figure}
            }
        \end{column}
        \begin{column}{0.3\linewidth}
            \only<3>{
                \begin{itemize}
                    \item J2 `twists' all orbits except polar
                    \item VERY high cost compared to Hohmann transfer
                \end{itemize}
            }
            \only<4>{
                \begin{itemize}
                    \item Similar to Hohmann, but small initial/final coasts
                    \item Impulses not parallel to velocity \(\implies\) plane change
                \end{itemize}
            }

            \only<5>{
                \begin{itemize}
                    \item 6 m/s more costly than Hohmann
                    \item Extra impulse: plane change
                    \item J2+Drag maneuver almost identical to J2 maneuver
                \end{itemize}
            }
        \end{column}
    \end{columns}
...
\begin{frame}
    \frametitle{Primer Vector circle to circle, J2+Drag}
    \pause
    \begin{itemize}
        \item Non-conservative model: only ODE method for primer vector calculation\pause
        \item In practice:
    \end{itemize}
    ...
    \begin{figure}[htbp]
        ...
        \begin{subfigure}{0.32\linewidth}
            \includegraphics[width=\linewidth]{../results/j2drag/hohmann/ICI_primer_vector.png}
        \end{subfigure}
        \begin{subfigure}{0.32\linewidth}
            \includegraphics[width=\linewidth]{../results/j2drag/hohmann/CICIC_primer_vector.png}
        \end{subfigure}
        \begin{subfigure}{0.32\linewidth}
            \includegraphics[width=\linewidth]{../results/j2drag/hohmann/CICICIC_primer_vector.png}
        \end{subfigure}
        % \caption{<caption>}
        % \label{<label>}
    \end{figure}
    ...
    \begin{itemize}
        \item STM method is indistinguishable from ODE method\pause
        \item \(\lVert \mathbf{A}_p - \mathbf{A}_\delta \rVert _\infty < 10^{-11}\)\pause
        \item \(\therefore\) drag is too weak! System is approx.\ conservative
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \end{itemize}\pause
        \item Primer vector algorithm can offer >100x decrase in cost\pause
        \item Short time frame: drag has little effect\pause
        \item Conjecture: 
...
\begin{frame}
    ...
    \pause
    \begin{itemize}
        \item \textbf{Accomplished} optimization of LEO impulsive maneuvers;
        \item \textbf{Extended} primer vector theory to perturbed models with 3 methods;\pause
        \item J2 affects nature of trajectories and maneuvers significantly;
        \item In the orbits and time scales considered, drag does not;\pause
        \item Future works
        \begin{itemize}
            \item Longer time spans
            \item Drag primer vector conjecture: differentiate STM and ODE methods
            \item \textbf{Estimate cost lower bound}: characterize gap to global opt.
        \end{itemize}
    \end{itemize}
    \pause
    \begin{center}
        {\LARGE Obrigado! Dúvidas?}
    \end{center}
